...
Query the industry name, aliases, technical routes, value-chain layer, business model, important companies, and relevant valuation terms. Open useful source cards rather than relying on index summaries.
...
Do not duplicate one finding across several new cards. Update existing objects when possible. Keep report detail and citations in the report archive; graph cards should compress the reusable conclusion and point back to sources.
